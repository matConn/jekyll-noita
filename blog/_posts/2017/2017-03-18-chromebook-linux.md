...
[Crouton](https://github.com/dnschneid/crouton) proved to be very easy to install and useful from the start. By creating a chroot and installing my distro of choice on it, this proved to be a fitting solution early on. I could run Ubuntu from the chroot either by switching over to it or accessing it from the Crosh command line.  
...
But now if I wanted to play a game in an emulator such as Mupen64plus or WINE, I could not run that from the command line while in ChromeOS; it would throw an error regarding X-server, forcing me to switch over to the chroot just for one program.  
...
After a full install, made easy thanks to GalliumOS's excellent documentation, and then finally a GalliumOS update, to my surprise, internal audio worked, and so I'm now using my Chromebook as I please, with the ability to install my favorite programs such as Visual Studio Code (a program I wanted from the very start), WINE, and all the rest; with that update came a patch, fixing my model's sound issue.  
